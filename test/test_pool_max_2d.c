...
int main()
{
	...
	// Copy memory
	memcpy(cnn->layerList[1].outMat.data.mat, src, sizeof(float) *
			cnn->layerList[1].outMat.data.rows *
	...
	memcpy(cnn->layerList[2].outMat.data.grad, desire, sizeof(float) *
			cnn->layerList[2].outMat.data.rows * cnn->layerList[2].outMat.data.cols);
	cnn_backward_pool(cnn->layerList, cfg, 2);

	printf("Gradient index mat:\n");
	print_img_int(cnn->layerList[2].pool.indexMat,
			cnn->layerList[2].outMat.width, cnn->layerList[2].outMat.height,
			cnn->layerList[2].outMat.channel);
	printf("\n");
